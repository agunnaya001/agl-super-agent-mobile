...
import { useGovernanceDiagnostic } from "../dao/lib";
...
export function GovernanceScreen() {
  ...
  const diagnostic = useGovernanceDiagnostic();
  ...
  return (
    <div>
      ...
      {/* Governance Diagnostic Connection Card */}
      <div className="card card-elev" style={{ borderColor: diagnostic.isHealthy ? "rgba(0,230,153,0.4)" : "rgba(255,184,0,0.4)", marginBottom: 12 }}>
        <div className="row between">
          <div className="row gap">
            <span className="status-dot" style={{ backgroundColor: diagnostic.isHealthy ? "var(--neon)" : diagnostic.isLoading ? "var(--cyan)" : "var(--gold)" }} />
            <span className="bold">Base Mainnet RPC Health</span>
          </div>
          <span className={`pill ${diagnostic.isHealthy ? "green" : diagnostic.isLoading ? "cyan" : "gold"}`}>
            {diagnostic.isLoading ? "PROBING..." : diagnostic.status.toUpperCase()}
          </span>
        </div>
        <div className="tiny muted" style={{ marginTop: 6 }}>
          {diagnostic.rpc?.endpoint ? `Active RPC: ${diagnostic.rpc.endpoint}` : "Probing Base Mainnet endpoints..."}
        </div>
        <div style={{ height: 10 }} />
        <div className="row between">
          <span className="small muted">Base Block & Latency</span>
          <span className="small bold">
            #{diagnostic.rpc?.blockNumber ? diagnostic.rpc.blockNumber.toLocaleString() : "..."} · {diagnostic.rpc?.latencyMs ?? 0}ms
          </span>
        </div>
        <div style={{ height: 6 }} />
        <div className="row between">
          <span className="small muted">Governor Contract</span>
          <span className="small bold" style={{ color: diagnostic.governor?.canInteract ? "var(--neon)" : "var(--gold)" }}>
            {diagnostic.governor?.canInteract ? "✅ Verified Online" : "⏳ Checking..."}
          </span>
        </div>
        <div style={{ height: 6 }} />
        <div className="row between">
          <span className="small muted">Timelock Contract</span>
          <span className="small bold" style={{ color: diagnostic.timelock?.canInteract ? "var(--neon)" : "var(--gold)" }}>
            {diagnostic.timelock?.canInteract ? "✅ Verified Bonded" : "⏳ Checking..."}
          </span>
        </div>
        <div style={{ height: 10 }} />
        <div className="row gap" style={{ marginTop: 4 }}>
          <button
            className="btn"
            style={{ fontSize: 11, padding: "5px 12px", background: "var(--cyan)", color: "#000", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 }}
            onClick={() => diagnostic.refetch()}
            disabled={diagnostic.isLoading}
          >
            {diagnostic.isLoading ? "Verifying..." : "🔄 Re-run Diagnostic"}
          </button>
          <button
            className="btn"
            style={{ fontSize: 11, padding: "5px 12px", background: "rgba(255,255,255,0.08)", border: "1px solid var(--border)", borderRadius: 6, cursor: "pointer" }}
            onClick={() => setShowLogs(!showLogs)}
          >
            {showLogs ? "Hide Logs" : `View Logs (${diagnostic.logs.length})`}
          </button>
        </div>

        {showLogs && (
          <div style={{ marginTop: 10, padding: 8, background: "rgba(0,0,0,0.35)", borderRadius: 6, maxHeight: 160, overflowY: "auto" }}>
            <div className="tiny bold muted" style={{ marginBottom: 4 }}>Diagnostic Audit Trail:</div>
            {diagnostic.logs.slice(-8).map((l, idx) => (
              <div key={idx} className="tiny mono" style={{ color: l.level === "error" ? "var(--rose)" : l.level === "success" ? "var(--neon)" : "var(--muted)", margin: "3px 0" }}>
